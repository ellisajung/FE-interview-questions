...
| No | 질문 |
| --- | --- |
| 1 | React의 useEffect 훅에서 의존성 배열(dependency array)의 역할은 무엇인가요? 의존성 배열을 잘못 관리했을 때 발생할 수 있는 질문점과, 이를 방지하기 위한 방법을 설명해주세요. |
| 2 | React에서 컨트롤드 컴포넌트(Controlled Component)와 언컨트롤드 컴포넌트(Uncontrolled Component)의 차이를 설명해주세요. |
| 3 | React에서 Context API를 사용하여 전역 상태를 관리할 때 발생할 수 있는 리렌더링 질문를 설명해주세요. |
| 4 | React에서 useReducer 훅과 useState 훅의 차이를 설명하고, 상태 관리에 useReducer를 사용하는 것이 적합한 상황을 예제와 함께 제시해주세요. |
| 5 | React에서 useRef 훅의 역할과 주요 사용 사례를 설명하고, useRef를 활용해 컴포넌트의 특정 DOM 요소에 접근하는 방법을 설명해주세요. |
| 6 | React에서 Suspense를 사용하는 방법과 주요 활용 사례를 설명해주세요. |
| 7 | React에서 서버 컴포넌트(Server Components)란 무엇인지 설명해주세요. |
| 8 | React에서 useLayoutEffect와 useEffect의 차이점을 설명하고, 각각의 훅이 적합한 상황을 예제와 함께 제시해주세요. |
| 9 | React에서 프로퍼티 드릴링(Prop Drilling)이란 무엇인지 설명하고, 이를 해결하기 위한 방법 3가지릉 제시해주세요. |
| 10 | React의 키(key) 속성은 어떤 역할을 하며, 컴포넌트 리스트를 렌더링할 때 key를 올바르게 설정하는 방법을 설명해주세요. |
| 11 | React의 동적 import를 사용하여 코드 스플리팅(Code Splitting)을 구현하는 방법을 설명해주세요. |
| 12 | React에서 에러 바운더리(Error Boundary)란 무엇이며 이를 구현하는 방법을 설명해주세요. |
| 13 | React에서 useMemo와 useCallback 훅의 차이를 메모이제이션과 함께 설명해주세요. |
| 14 | React에서 Strict Mode란 무엇이며, 이를 사용하는 목적과 주요 기능을 설명해주세요. |
| 15 | React의 forwardRef에 대해 설명하고, 이를 활용하여 하위 컴포넌트의 DOM 요소에 접근할 수 있는 예제를 작성해주세요. |
| 16 | React에서 Custom Hook이란 무엇인지, 어떻게 설계하는지 설명해주세요. |
| 17 | React에서 컨트롤드 컴포넌트(Controlled Component)와 언컨트롤드 컴포넌트(Uncontrolled Component)의 차이를 설명하고 각각의 장단점을 비교해주세요. |
| 18 | React에서 HOC(Higher-Order Component)란 무엇이며, 이를 사용하여 인증된 사용자만 접근할 수 있는 컴포넌트를 구현하는 방법을 설명해주세요. |
| 19 | React에서 리렌더링(Re-rendering)이 발생하는 조건을 아는 한 모두 설명해주세요. |
| 20 | React에서 동적 스타일링(Dynamic Styling)을 적용하는 방법을 3가지 이상 설명하고, 각각의 장단점을 비교해주세요. |
| 21 | React에서 컨텍스트(Context API)란 무엇이며 이를 사용하여 전역 상태 관리를 구현하는 방법을 설명해주세요. |
| 22 | React에서 Strict Mode가 useEffect의 실행에 미치는 영향을 설명하고 이를 처리하는 방법을 설명해주세요. |
| 23 | React에서 hydration(하이드레이션)이란 무엇이며, 이를 사용하는 이유를 설명해주세요. |
| 24 | React에서 키(key) 속성의 역할을 설명하고, 리스트 렌더링 시 index를 key로 사용하는 것이 질문가 될 수 있는 이유를 설명해주세요. |
| 25 | React에서 포털(Portal)이란 무엇이며, 이를 사용해야 하는 이유를 예시를 들어 설명해주세요. |
| 26 | React의 이벤트 위임(Event Delegation)이란 무엇이며, React에서 기본적으로 이벤트가 어떻게 처리되는지 설명해주세요. |
| 27 | React에서 useReducer 훅이 무엇이며, useState와의 차이점을 설명해주세요. |
| 28 | React에서 forwardRef는 무엇이며, 어떤 상황에서 유용하게 사용할 수 있는지 설명해주세요. |
| 29 | React의 useLayoutEffect와 useEffect의 차이점을 설명하고, 각각 언제 사용해야 하는지 설명해주세요. |
| 30 | React에서 에러 바운더리(Error Boundary)란 무엇이며, 이를 활용하여 컴포넌트 트리에서 발생하는 오류를 안전하게 처리하는 방법을 설명해주세요. |
| 31 | React에서 컨텍스트(Context API)와 전역 상태 관리 라이브러리(Redux, Zustand 등)의 차이점을 설명해주세요. |
| 32 | React에서 커스텀 훅(Custom Hook)이란 무엇이며, 이를 사용해야 하는 이유를 설명해주세요. |
| 33 | React에서 컴포넌트 컴포지션(Component Composition)이란 무엇이며 이를 활용하는 장점을 설명해주세요. |
| 34 | React에서 useEffect의 의존성 배열([], [value], 없음)의 차이점을 설명해주세요. |
| 35 | React에서 key 속성의 역할과 중요성과 리스트 렌더링(map) 시 key를 올바르게 사용하는 방밥을 설명해주세요. |
| 36 | React에서 Context API와 Redux의 차이점을 설명해주세요. |
